...
export function SectionHeading({
  ...
  return (
    <div className={`mb-10 ${center ? "text-center" : ""}`}>
      ...
      <h2 className="mt-1 font-display text-4xl font-semibold text-cream sm:text-5xl">{title}</h2>
      <div className={`mt-4 h-px w-24 gold-line ${center ? "mx-auto" : ""}`} />
      {sub && <p className={`mt-4 max-w-2xl text-sm leading-relaxed text-muted sm:text-base ${center ? "mx-auto" : ""}`}>{sub}</p>}
    </div>
  );
}

export function AboutSection({ content, full }: { content: Content; full?: boolean }) {
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-2">
        <div className="relative">
          ...
          <div className="absolute -bottom-5 -right-3 rounded-xl border border-gold/30 bg-ink/95 px-5 py-3 shadow-xl backdrop-blur sm:-right-5">
            <p className="font-script text-2xl text-gold2">Good Food, Good Mood</p>
          </div>
        </div>
        <div>
  ...
  return (
    <section className="py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionHeading script="Special Offers" title="Something Special Awaits You" />
        {offers.length === 0 ? (
          <p className="text-center text-muted">No active offers right now — check back soon.</p>
        ) : (
            ...
            {offers.map((o) => (
              ...
              >
                <div className="relative h-52 overflow-hidden">
                  <Img src={o.image} alt={o.title} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
...
export function GallerySection({ images, limit }: { images: GalleryImage[]; limit?: number }) {
  ...
  return (
    <section className="py-20 sm:py-28 bg-ink/40">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionHeading script="Gallery" title="Moments & Flavours" sub="A glimpse inside China Garden — our kitchen, our dining rooms and the celebrations we host." />
        {shown.length === 0 ? (
          ...
          <div className="columns-2 gap-4 md:columns-3 [column-fill:_balance]">
            {shown.map((g, i) => (
              ...
              >
                <Img src={g.url} alt={g.caption || "China Garden"} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
      ...
      {active && (
        <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/90 p-4 anim-fade-in" onClick={() => setActive(null)}>
          <button className="absolute right-4 top-4 rounded-full border border-white/20 p-2 text-white/80 hover:text-white" aria-label="Close">
            <Icon name="x" />
          </button>
          <figure className="max-h-full max-w-4xl" onClick={(e) => e.stopPropagation()}>
            <img src={active.url} alt={active.caption || "China Garden"} className="max-h-[82vh] w-auto rounded-lg object-contain shadow-2xl" />
            {active.caption && <figcaption className="mt-3 text-center text-sm text-muted">{active.caption}</figcaption>}
          </figure>
        </div>
      )}
    </section>
...
export function ContactSection({ content }: { content: Content }) {
  ...
  return (
    <section className="py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionHeading script="Find Us" title="Visit China Garden" sub={content["contact.address"]} />
        <div className="grid gap-8 lg:grid-cols-5">
          <div className="space-y-4 lg:col-span-2">
            {[
              { icon: "pin" as const, label: "Address", value: content["contact.address"], href: content["contact.mapsUrl"] },
              { icon: "phone" as const, label: "Phone", value: content["contact.phone"], href: `tel:${(content["contact.phone"] || "").replace(/[^+\d]/g, "")}` },
              { icon: "whatsapp" as const, label: "WhatsApp", value: "Chat with us instantly", href: waLink(wa, "Hello China Garden! 👋") },
              { icon: "facebook" as const, label: "Facebook", value: "Follow our page", href: content["contact.facebook"] },
              { icon: "mail" as const, label: "Email", value: content["contact.email"], href: `mailto:${content["contact.email"]}` },
            ]
              .filter((c) => c.value)
              .map((c) => (
                <a
                ...
                >
                  ...
                  <span>
                    ...
                    <span className="block text-sm font-semibold text-cream">{c.value}</span>
                  </span>
                </a>
              ))}
            <div className="rounded-xl border border-line bg-ink p-4">
              <p className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted">
                <Icon name="clock" className="w-4 h-4 text-gold" /> Opening Hours
              </p>
              <ul className="space-y-1.5 text-sm">
                {hours.map((h) => (
                  <li key={h.day} className="flex justify-between text-muted">
                    <span>{h.day}</span>
                    <span className={h.closed ? "text-red-400" : "text-cream/80"}>{h.closed ? "Closed" : `${h.open} – ${h.close}`}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <a href={waLink(wa, "Hello China Garden! 👋")} target="_blank" rel="noreferrer"
          ...
          <div className="overflow-hidden rounded-2xl border border-line lg:col-span-3">
            {content["contact.mapsEmbed"] ? (
              ...
              <div className="flex h-full min-h-[420px] items-center justify-center bg-ink2 text-muted">Map not configured</div>
            )}
          </div>
        </div>
      </div>
